...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.eval_interval == 0:
            ...
            plt.tight_layout()
            plt.savefig(f"eigen_e{epoch}.tiff", transparent=True, dpi=args.dpi)
            plt.close()
            ...
            if epoch > 0:
                for key, value in stats.items():
                    ...
                    plt.tight_layout()
                    plt.savefig(f"loss_{key}_e{epoch}.tiff", transparent=True, dpi=args.dpi)
                    plt.close()
            ...
            plt.savefig(f"simmat_sc_e{epoch}.tiff", transparent=True, dpi=args.dpi)
            plt.close()
            ...
            for i, (label, indices) in enumerate(samples_fec.items()):
                ...
                if i % 5 == 4:
                    ...
                    plt.tight_layout()
                    plt.savefig(f"samples_{i // 5}_e{epoch}.tiff", transparent=True, dpi=args.dpi)
                    plt.close()
            ...
            plt.tight_layout()
            plt.savefig(f"simrank_e{epoch}.tiff", transparent=True, dpi=args.dpi)
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(f"cm_sc_e{epoch}.tiff", transparent=True, dpi=args.dpi)
            plt.close()
        ...
        if epoch % args.embedding_interval == 0:
            ...
            ax.set_aspect(1)
            plt.tight_layout()
            plt.savefig(f"qz_true_e{epoch}.tiff", transparent=True, dpi=args.dpi)
            plt.close()
            ...
            ax.set_aspect(1)
            plt.tight_layout()
            plt.savefig(f"qz_sc_e{epoch}.tiff", transparent=True, dpi=args.dpi)
            plt.close()
