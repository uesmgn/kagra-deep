        ...
        if epoch % args.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                for j in range(args.num_heads):
                    ...
                    plt.title(r"confusion matrix $y$ with $q(y)$ by heads-{} at epoch-{}".format(j, epoch))
                    ...
                    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", cbar=False, yticklabels=targets)
                    plt.yticks(rotation=45)
                    plt.title(r"confusion matrix $y$ with $q(w)$ by heads-{} at epoch-{}".format(j, epoch))
                ...
                plt.title(r"$q(z)$ at epoch-{}".format(epoch))
                plt.tight_layout()
                ...
                for j in range(args.num_heads):
                    ...
                    plt.legend(bbox_to_anchor=(1.01, 1.0), loc="upper left")
                    plt.title(r"$q(z)$ labeled by heads-{} at epoch-{}".format(j, epoch))
                    plt.tight_layout()
