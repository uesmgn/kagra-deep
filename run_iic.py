...
def main(args):
    ...
    train_set = datasets.co(train_set, transform_fn, augment_fn, l=args.l)
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, *ax), _ in tqdm(train_loader):
            x = x.to(device)
            mi, mi_over = model(x, *ax, lam=args.lam, detach=args.z_detach)
            loss = sum([mi, mi_over])
            optim.zero_grad()
            loss.backward()
            optim.step()
