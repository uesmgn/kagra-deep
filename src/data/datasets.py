...
class HDF5_C(data.Dataset):
    def __init__(self, root, transform, augment, target_transform, target_tag="target", shape=None):
        super().__init__()
        self.__root = os.path.abspath(root)
        if not os.path.isfile(self.__root):
            raise FileNotFoundError(f"No such file or directory: '{self.__root}'")
    ...
    def __getitem__(self, i):
        ...
        if self.augment is not None:
            x_augmented = self.augment(x)
        if self.__target_transform is not None:
            target = self.__target_transform(target)
        return x_transformed, x_augmented, target
