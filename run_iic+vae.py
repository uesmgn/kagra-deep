...
model = models.IIC_VAE(flags.model, in_channels=4,
...
for epoch in range(1, flags.num_epochs):
    ...
    for x, targets in tqdm(train_loader):
        x = x.to(device)
        x_densed = model(x)
        xt_densed = model(x, perturb=True)

        # get IIC loss
        y_outputs, y_over_outputs = model.iic(x_densed)
        yt_outputs, yt_over_outputs = model.iic(xt_densed)
        ...
        for i in range(flags.num_heads):
            ...
            loss_step_head = model.iic_loss(y, yt) + model.iic_loss(y_over, yt_over)
        ...
        loss_step_iic = torch.sum(loss_step_for_each_head)
        if flags.avg_for_heads:
            loss_step_iic /= flags.num_heads

        # get VAE loss
        x_generated, z, z_mean, z_var = model.vae(x_densed)
        loss_step_vae = model.vae_loss(x, x_generated, z_mean, z_var)

        loss_step = loss_step_iic + loss_step_vae
        ...
        loss["train_iic"] += loss_step_iic.item()
        loss["train_vae"] += loss_step_vae.item()

    scheduler.step()
    log['train_loss'].append(loss['train'])
    log['train_loss_iic'].append(loss['train_iic'])
    log['train_loss_vae'].append(loss['train_vae'])
    best_head_idx = head_selecter.argmin(dim=-1).item()

    print(f'train loss: {loss["train"]:.3f}')
    print(f'train iic loss: {loss["train_iic"]:.3f}')
